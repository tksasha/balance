package handlers_test //nolint:dupl
...
func TestGetItemsHandler_GetItems(t *testing.T) {
	...
	t.Run("when GetItems returns an error it should respond with 500 code", func(t *testing.T) {
		itemService.EXPECT().GetItems(ctxWithValue).Return(nil, errors.New("get items error"))
	...
	t.Run("when GetItems doesn't return any error it should respond with 200 code", func(t *testing.T) {
		itemService.EXPECT().GetItems(ctxWithValue).Return(models.Items{}, nil)
